...
function Test-ImageExistsInBothPublicAndPrivateRepos($release) {
  return ($privateImages -contains $release) -and ($publicImages -contains $release)
}

function Test-ImageExistsInPrivateRepo($release) {
  return ($privateImages -contains $release)
}

function Publish-PrivateImageToPublicRepo($release) {
...
function Start-ImageBuildFromScratch($release) {
...
foreach($release in $releases) {
  if (Test-ReleaseShouldBeRebuilt $release) {
    ...
    Start-ImageBuildFromScratch $release
  }
  elseif (Test-ImageExistsInBothPublicAndPrivateRepos $release) {
    write-host "Docker image for Octopus Server $release exists in both public and private repositories. Nothing to do."
  }
  elseif (Test-ImageExistsInPrivateRepo $release) {
    Publish-PrivateImageToPublicRepo $release
  }
  else {
    Start-ImageBuildFromScratch $release
  }
